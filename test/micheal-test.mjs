...
describe("micheal", () => {
  it.skip("callMicheal", () => {
    describe("module | callMicheal", () => {
      it.skip("gets a Michael", async () => {
        let myMichael = await callMicheal()
        myMichael.should.eql({ potentialAction: "Action" })
      })
      it.skip("gets a Michael and give argv to `thing`", async () => {
    ...
    describe("module | michael", () => {
      it.skip("michael `blank` with no `actions`", async () => {
        let michaeledThing = await michael()
        michaeledThing.should.eql({})
      })
      it.skip("michael processes a pipeline", async () => {
        let pipelineActions = [
          {
            Action: {
              target: thing =>
    ...
    describe("module | pipeline", () => {
      ...
      it.skip("pipeline promise endpoints", async () => {
        let MS = 50
        let start = Date.now()
        await pipeline(
          { identifier: "testMicheal" },
